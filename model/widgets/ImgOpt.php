 ...
 *
 * ```php
 * <?= \widgets\ImgOpt::widget(["src" => "/images/product/extra.png", "alt" => "Extra product" ]) ?>
 * ```
 *
...
class ImgOpt extends Widget
{
	...
	private function get_or_convert_to_webp($img, $recreate = false)
	{
		...
		$web_root = Yii::getAlias('@staticRoot');
	...
	public function init()
	{
		...
		$this->src = str_replace(Yii::getAlias('@staticRoot'), '', $this->src);

		$webp_path = $this->get_or_convert_to_webp($this->src, (self::RECREATE_ALL == true || $this->recreate == true));

        $this->_webp = Yii::getAlias('@static').$webp_path;
		$this->src = Yii::getAlias('@static'). $this->src;
		...
		if ($this->lightbox_data)
		{
			...
			if ($this->lightbox_src === null)
			{
                ...
                $this->lightbox_src = str_replace(Yii::getAlias('@staticRoot'), '', $this->lightbox_src);
                $lightbox_webp_path = $this->get_or_convert_to_webp($this->lightbox_src, (self::RECREATE_ALL == true || $this->recreate == true));
                $this->lightbox_src = Yii::getAlias('@static'). $this->lightbox_src;
            }
	...
	public function run()
	{
		...
		if ($this->lightbox_data)
		{
			return Html::a($html, $this->lightbox_src, [ "data-toggle" => $this->lightbox_data, "data-title" => $this->lightbox_title, "data-gallery" => "gallery" ] );
		}
